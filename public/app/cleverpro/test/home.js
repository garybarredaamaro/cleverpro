(function(){
	...
	TestController.$inject = ['TaskService', 'UserSession', 'ModuleService', '$stateParams', '$state'];

	function TestController(TaskService, UserSession, ModuleService, $stateParams, $state){
		var vm = this;
		...
		function run(){
			vm.loading = true;
			TaskService.run().then(function(data){
				vm.loading = false;
				$state.go('welcome.mytasks');
			});
		}
